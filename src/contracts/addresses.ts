...
export function resolveAsset(
  asset: string,
  config: typeof TYDRO_ADDRESSES.mainnet
): { address: string; decimals: number } {
  if (asset.startsWith('0x')) {
    // Raw address — find decimals from registry or default 18
    const match = Object.values(config.assets).find(
      (a) => a.address.toLowerCase() === asset.toLowerCase()
    );
    if (!match) {
      throw new Error(
        `Address ${asset} is not in the Tydro reserve registry. ` +
  ...
  const entry = (config.assets as Record<string, { address: string; decimals: number }>)[key];
  if (!entry) {
