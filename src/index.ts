...
const NETWORK = (process.env.TYDRO_NETWORK ?? 'mainnet') as keyof typeof TYDRO_ADDRESSES;
...
if (NETWORK === 'testnet') {
  throw new Error(
    'Testnet addresses are not yet configured. Set TYDRO_NETWORK=mainnet or contribute testnet addresses.'
  );
}
...
function validateAddress(addr: string, fieldName: string): string {
...
function validateAmount(amount: string, fieldName = 'amount'): void {
...
async function ensureAllowance(
  ...
  const current: bigint = await token.allowance(await signer.getAddress(), spender);
  if (current >= requiredAmount) return; // already sufficient
  ...
  tools: [
    {
      ...
      description:
        'Get supply APY, borrow APY, total liquidity, utilization, and available liquidity for any Tydro reserve. ' +
    ...
    },
    {
      name: 'supply',
      description:
        'Supply (deposit) an asset to Tydro to earn interest. Requires PRIVATE_KEY env var. ' +
    ...
    },
    {
      name: 'borrow',
      description:
        'Borrow an asset from Tydro. Requires existing collateral and PRIVATE_KEY env var. ' +
  ...
  try {
    ...
    switch (name) {
      ...
      case 'get_reserve_data': {
        if (!a.asset) throw new Error('Missing required field: asset');
        const { address: assetAddress, decimals } = resolveAsset(a.asset, mainnetConfig);

        const dataProvider = new ethers.Contract(
          mainnetConfig.contracts.PoolDataProvider,
          DATA_PROVIDER_ABI,
          provider
        );
      ...
      case 'get_user_account': {
        if (!a.address) throw new Error('Missing required field: address');
        ...
        const healthFactorRaw = data.healthFactor as bigint;
      ...
      // ── supply ──────────────────────────────────────────────────────────────
      case 'supply': {
        if (!signer) throw new Error('PRIVATE_KEY not set — read-only mode. Set PRIVATE_KEY to execute transactions.');
        if (!a.asset) throw new Error('Missing required field: asset');
        if (!a.amount) throw new Error('Missing required field: amount');
